  ...
  proxyReq.on('response', (proxyRes) => {
    ...
    if (contentType?.includes('stream')) {
      ...
      proxyRes.on('data', chunk => {
        ...
        // FIX: Count tokens from this chunk's delta incrementally
        // All delta fields except 'role' are generated by AI and should be counted
        try {
          ...
          for (const line of lines) {
            if (line.startsWith('data:')) {
              ...
              try {
                ...
                if (delta) {
                  // Count tokens from all delta fields except 'role'
                  let accumulatedContent = '';
                  for (const key in delta) {
                    if (key !== 'role' && delta[key] !== undefined && delta[key] !== null) {
      ...
      proxyRes.on('end', () => {
        ...
        // FIX: completionTokens is now the accumulated count from all chunks
